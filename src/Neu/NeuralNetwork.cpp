...
NeuralNetwork::NeuralNetwork(std::initializer_list<size_t> layers) {
	...
	m_cost_f = LossFunctions::mean_sqrd;
	m_cost_der_f = LossFunctions::mean_sqrd_der;
...
void NeuralNetwork::train()
{
	...
	for (size_t i = 0; i < m_epochs; i++)
	{
		...
		for (size_t j = 0; j < m_sampleSize / m_batchSize; j++)
		{
			...
			calculateBatchError();
			float loss = std::accumulate(m_batch_error, m_batch_error + m_outputSize, 0.f);
...
float* NeuralNetwork::calculateBatchErrorDelta(const float* expected)
{
	...
	for (size_t i = 0; i < m_batchSize; i++)
	{
		...
		std::transform(m_error_delta, m_error_delta + m_outputSize, m_batch_error_delta, m_batch_error_delta, std::plus<float>());
	}
